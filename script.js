      ...
      <footer class="fixed-footer">
        <div class="footer-content" style="font-size: 0.9rem;">
            © 2025 nobetplani.com | Geliştirici: <a href="hakkimizda.html" style="color: #b2dfdb; text-decoration: underline;">Dr. M. Celil Aslan</a>
            <span style="margin: 0 10px;">|</span>
            <a href="gizlilik.html" style="color: #fff; text-decoration: underline;">Gizlilik Politikası ve Şartlar</a>
            
            <a href="https://www.instagram.com/nobetplani" target="_blank" style="margin-left: 15px; display: inline-flex; align-items: center;">
                <img src="instagram-logo.png" style="width: 20px; height: 20px; margin-right: 5px;"> Instagram
            </a>
        </div>
    </footer>
...
<script>
    ...
    const firebaseConfig = {
      apiKey: "AIzaSyCT5W8yb_lEhXgXDqc_m95IgZ71ogtwpks",
      authDomain: "nobet-plani.firebaseapp.com",
      projectId: "nobet-plani",
      storageBucket: "nobet-plani.firebasestorage.app",
      messagingSenderId: "280684468506",
      appId: "1:280684468506:web:963c805d69c9ef1e00f4a3",
      measurementId: "G-MK1WG1ZLRD"
    };
    ...
    function loadHistoryList() {
        const user = auth.currentUser;
        if (!user) {
            document.getElementById('historyList').innerHTML = '<a class="collection-item">Görüntülemek için giriş yapmalısınız.</a>';
            return;
        }
            ...
            .then((querySnapshot) => {
                ...
                sortedYears.forEach(year => {
                    ...
                    if (isCurrentOrNewest) {
                        ...
                        yearHeader.querySelector('i:last-child').innerText = 'expand_less'; // Ok yukarı baksın
                        yearHeader.classList.add('teal', 'lighten-5'); // Aktif yıl 
                    } else {
                        monthsContainer.style.display = 'none';
                    }

                    // Başlığa Tıklama Olayı
                    yearHeader.onclick = () => {
                    ...
                    // --- AYLARI LİSTELE ---
                    // Ayları yeniden eskiye sıralayalım (Aralık -> Ocak)
                    ...
                    yearsMap[year].forEach(data => {
                        ...
                        li.className = 'collection-item waves-effect black-text'; // EKLENDİ
                        li.style.paddingLeft = '45px'; 
                        li.style.borderLeft = '4px solid transparent'; 
                        li.style.color = '#374151'; // EKLENDİ (Varsayılan koyu renk)
                        li.innerHTML = `
                        ...
                        li.onclick = function() {
                            ...
                            // Seçili olduğunu belli et (Soluna renkli çizgi koy)
...
// Veriyi Tabloya Çeviren Fonksiyon
function renderMagicList(data) {
    const assignments = JSON.parse(data.assignments || '{}');
    const personnel = JSON.parse(data.personnelSnapshot || '[]');
    const holidays = (data.holidays || "").split(',').map(d => parseInt(d)).filter(n => !isNaN(n));
    ...
    // Tabloyu Çiz
    let html = '<table class="striped bordered centered" style="font-size: 0.85rem;"><thead><tr><th style="min-width: 80px;">İsim</th>';
    for(let d=0; d<daysDiff; d++){
        const current = new Date(startDate);
        current.setDate(startDate.getDate() + d);
        html += `<th style="padding: 5px; min-width: 25px;">${current.getDate()}</th>`;
    }
    html += '</tr></thead><tbody>';

    personnel.forEach((p, pIndex) => {
        html += `<tr><td style="font-weight:bold; text-align:left; padding-left: 5px;">${p.name}</td>`;
        for(let d=0; d<daysDiff; d++){
            const isAssigned = assignments[`${pIndex}-${d}`];
            const current = new Date(startDate);
            current.setDate(startDate.getDate() + d);
            const isWknd = current.getDay() === 0 || current.getDay() === 6 || holidays.includes(current.getDate());
            
            let cellStyle = isWknd ? "background-color: #eceff1;" : "";
            let cellContent = isAssigned ? '<span style="color: green; font-weight: bold;">✓</span>' : '';
    ...
    document.getElementById('magicLinkCalendarContainer').innerHTML = html;
}
